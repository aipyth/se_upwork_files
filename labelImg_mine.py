...
# Aipyth 20180527: commended cause I dunno what this library is for
# import resources
# Add internal libs
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        # Open last foulder and image
        if self.openLastFoulder():
            self.openLastImgPath()
    ...
    def loadFile(self, filePath=None, saveFlag=True):
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            if saveFlag:
                self.saveLastImgPath(filePath)
            return True
        return False

    def saveLastImgPath(self, filePath):
        if self.mayContinue():
            currPath = os.getcwd()
            ...
            os.chdir('cache')
            with open('lastimg', 'w') as file:
                file.write(filePath)
            os.chdir(currPath)
            return True
        return False

    def openLastImgPath(self):
        if self.mayContinue():
            currPath = os.getcwd()

            try:
                os.chdir('cache')
                with open('lastimg', 'r') as file:
                    imgPath = file.read()
            except FileNotFoundError:
                os.chdir(currPath)
                return False

            self.loadFile(imgPath, saveFlag = False)

            os.chdir(currPath)
            return True
        return False

    def saveLastFoulder(self, path):
        if self.mayContinue():
            currPath = os.getcwd()
    ...
    def openLastFoulder(self):
        if self.mayContinue():
            currPath = os.getcwd()

            try:
                os.chdir('cache')
                with open('lastdir', 'r') as file:
                    dirPath = file.read()
            except FileNotFoundError:
                os.chdir(currPath)
                return False
    ...
    def openDirDialog(self, _value=False, dirpath=None):
        ...
        self.saveLastFoulder(targetDirPath)
    ...
    def openNextImg(self, _value=False):
        ...
        if filename:
            self.loadFile(filename)
            # Aipyth 20180528: Forces to create rect after 'd' pressed
            self.createShape()
