...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.lastimg = 'lastimg.txt'
        if defaultFilename == None:
            try:
                lastimg = open(ustr(self.lastimg), 'r')
            except:
                print('lastimg.txt is not exist')
            else:
                with lastimg:
                    lastimgname = os.path.normpath(lastimg.readline())
                    defaultFilename = lastimgname if os.path.isfile(lastimgname) else None
                    dirpath = os.path.dirname(defaultFilename) if defaultFilename is not None else None
                    if dirpath is not None: # added by SE
        ...
        openf = action('&Open', self.openFile,
        ...
        self.actions = struct(save=save, save_format=save_format, saveAs=saveAs, open=openf, close=close, resetAll = resetAll,
    ...
    def newShape(self):
        ...
        if text is not None:
            ...
            #shape = self.canvas.setLastLabel(text, QColor(*[x for x in generate_color.getRgb()[0:3]], 255), generate_color)
            self.addLabel(shape)
    ...
    def loadFile(self, filePath=None):
        ...
        # Save filePath into txt file
        with open(ustr(self.lastimg), 'w') as lastimg:
            lastimg.write(os.path.normpath(filePath))
    ...
    def openPrevImg(self, _value=False):
        ...
        #if self.autoSaving.isChecked():        # se
        if self.defaultSaveDir is not None:     # un indent se
            if self.dirty is True:          # un indent se
                self.saveFile()             # un indent se
        else:                               # un indent se
            self.changeSavedirDialog()      # un indent se
            return                          # un indent se
    ...
    def openNextImg(self, _value=False):
        # Proceding next image without dialog if having any label
        #if self.autoSaving.isChecked(): #se
        if self.defaultSaveDir is not None:  # unindent se
            if self.dirty is True:      # unindent se
                self.saveFile()         # unindent se
        else:                           # unindent se
            self.changeSavedirDialog()  # unindent se
            return                      # unindent se
